        ...
        if (val.address == reversedGeoAddress) {
          ...
          $.ajax({
            ...
            success: function(response) {
              ...
              var objectLocation = JSON.parse(response[0].location);
              ...
              geocodeService.reverse().latlng(objectLocation).run(function(error, result) {
                ...
                $.ajax({
                  type: "GET",
                  url: "https://localhost:3000/theme/" + response[0].themeID,
                  success: function(theme) {

                $("#vote_text").text(response[0].votes);
                $("#objectPosition").text(reversedGeoAddress);
                $("#objectCreated").text(new Date(response[0].createdAt).toLocaleString());
                $("#objectUpdated").text(new Date(response[0].updatedAt).toLocaleString());
                  $("#objectCategories").text(JSON.parse(response[0].categories));
                $("#objectTheme").text(theme[0].name);
                $("#objectDescription").text(response[0].description);
              ...
              error: function(err) {
                ...
                jQuery.each(response[0].categories, function(i, cat) {
                  $("#modal_marker_edit #tagsinput_edit").tagsinput('add', cat);
                });


                $("#modal_marker_edit #textarea_infos_edit").val(response[0].description);
        ...
        markers[i].on('click', function() {
          ...
          geocodeService.reverse().latlng(JSON.parse(val.location)).run(function(error, result) {
            ...
            markers[i].bindPopup(reversedGeoAddress + "<br>" + "<img id='markerImage' src='https://images-na.ssl-images-amazon.com/images/I/61vWHzU8L5L._SY355_.jpg'/>" + "<br>" +
              "<a id='popup_link' href='#modal_marker' target='_self' data-toggle='modal'>Weitere Informationen</a>").openPopup();
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          geocodeService.reverse().latlng(JSON.parse(response.location)).run(function(error, result) {
            ...
            $("#objectCategories").text(JSON.parse(response.categories));
          ...
          error: function(err) {
            ...
            $("#modal_marker #tagsinput").tagsinput('removeAll');
            $("#modal_marker #myposition").html(reversedGeoAddress);

            jQuery.each(JSON.parse(response.categories), function(i, vale) {
              $("#modal_marker #tagsinput").tagsinput('add', vale);
            });


            $("#modal_marker #textarea_infos").val(response.description);
